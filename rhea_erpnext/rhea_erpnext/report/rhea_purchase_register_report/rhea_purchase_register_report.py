...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	purchase_inv = list(set([d.name for d in invoice_list]))
	payment_details = get_payment_details(purchase_inv)
	...
	for inv in invoice_list:
		...
		row += [total_tax, inv.base_grand_total, flt(inv.base_grand_total, 2), inv.outstanding_amount, payment_details.get(inv.name)]
		data.append(row)
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = columns + expense_columns + [_("Net Total") + ":Currency/currency:120"] + tax_columns + \
		[_("Total Tax") + ":Currency/currency:120", _("Grand Total") + ":Currency/currency:120",
			_("Rounded Total") + ":Currency/currency:120", _("Outstanding Amount") + ":Currency/currency:120",_("Payment Reference No") + "::150"]
...
def get_payment_details(purchase_inv):
	payment_details = {}
	for pay in frappe.db.sql(""" select  A.reference_no, A.name, A.parent, B.reference_name 
		from `tabPayment Entry` as A inner join `tabPayment Entry Reference` as B 
		ON A.name=B.parent where B.reference_name in (%s) and B.docstatus = 1""" % ", ".join(["%s"]*len(purchase_inv)), tuple(purchase_inv), as_dict=1):
			payment_details.setdefault(pay.reference_name, pay.name)
	for x in xrange(1,10):
		print("dsfsf", payment_details)
